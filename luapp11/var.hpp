...
/**
 * A lua "variable".  A specific place in the lua environment which can be read
 * from and assigned to.
 */
class var {
 public:
 ...
 private:
  ...
  template <typename T>
  struct typed_is<T, typename std::enable_if<
...
class var_const_iterator : public std::iterator<std::forward_iterator_tag,
                                                std::pair<val, const var>> {
 public:
  var_const_iterator(const var& v) : v_(&v), L(v.L), g_(new stack_guard(L)) {
    v.push();
    idx_ = lua_gettop(L);
    lua_pushnil(L);
    inc();
  }
  var_const_iterator(const var_const_iterator&) = delete;
  var_const_iterator(var_const_iterator&& other) { swap(*this, other); }
  var_const_iterator& operator++() {
    inc();
    return *this;
  }
  bool operator==(const var_const_iterator& other) {
    return std::tie(L, idx_) == std::tie(other.L, other.idx_);
  }
  bool operator!=(const var_const_iterator& other) {
    return std::tie(L, idx_) == std::tie(other.L, other.idx_);
  }
  var_const_iterator& operator=(var_const_iterator other) {
    swap(*this, other);
    return *this;
  }
  std::pair<val, const var>& operator*() { return pair_; }
  std::pair<val, const var>* operator->() { return &pair_; }
  friend void swap(var_const_iterator&& a, var_const_iterator&& b)  // nothrow
  {
    using std::swap;
 ...
 private:
  var_const_iterator(lua_State* L, int idx) : L(L), idx_(idx), v_(nullptr) {}
  void inc() {
    if (!lua_next(L, idx_)) {
      idx_ = 0;
    }
